...
checkpoint = ModelCheckpoint(
    filepath=os.path.join(MODEL_DIR, "qa-lstm-fem-attn-best.hdf5"),
...
model.save_weights(os.path.join(MODEL_DIR, "qa-lstm-fem-attn-final.hdf5"))
with open(os.path.join(MODEL_DIR, "qa-lstm-fem-attn.json"), "wb") as fjson:
    fjson.write(model.to_json())

model.load_weights(filepath=os.path.join(MODEL_DIR, 
                                         "qa-lstm-fem-attn-best.hdf5"))
loss, acc = model.evaluate([Xqtest, Xatest], Ytest, batch_size=BATCH_SIZE)
